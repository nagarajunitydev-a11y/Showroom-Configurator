...
export const ClientGridPage = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.16),transparent_38%),linear-gradient(135deg,_#05070b_0%,_#090b11_55%,_#020304_100%)] p-4 font-sans text-white sm:p-6 lg:p-8">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-7xl flex-col">
        <header className="mb-8 flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-black/30 px-5 py-5 backdrop-blur-xl sm:flex-row sm:items-end sm:justify-between sm:px-7">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-zinc-300 transition-colors hover:border-white/20 hover:text-white"
          >
            Back home
          </button>
        </header>
            ...
            {visibleVehicles.map((vehicle, index) => {
              ...
              return (
                <div key={vehicle.id} className="absolute z-20" style={position}>
                  <VehicleCard vehicle={vehicle} onSelect={initializeConfigurator} index={index} />
                </div>
              );
            })}
